...
namespace TorchClassifier
{
    ...
    internal class NamesClassifier
    {
        ...
        private readonly int _maxTrainCount = 20000;


        protected List<string> _categories;

        protected List<NamesClassifierRecord> _records;
        ...
        public IEnumerable<object> Predict(RnnModule rnn, string line, int maxPredictions = 2)
        {
            ...
            for (var i = 0; i < maxPredictions; i++)
            {
                ...
                Console.WriteLine($"({value}) {_categories[numCategoryIndex]}");
        ...
        public void Run()
        {
            ...
            for (var i = 0; i < _maxTrainCount; i++)
            {
                ...
                if (i % 1000 == 0)
                {
                    ...
                    Console.WriteLine($"{percent:D}% ({i} of {_maxTrainCount}) loss={loss} line[{randomIndex}]={randomRecord.Name} / {guess} {correct}");
                }
            }

            var testNames = new [] { "Марьяна", "Лена", "Ульяна", "Антон", "Сергей", "Света"};
            Console.WriteLine("\nTesting predict capabilities:");
            foreach (var testName in testNames)
            {
                Console.WriteLine($"Test word: \"{testName}\"");
                Predict(rnn, testName);
            }
        }
